...
const Feed = ({ companyId, limit }: Props) => {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      if (companyId) {
        if (filter === "recent") {
          const followingPosts = await getFollowingFeed();
          data = followingPosts.filter(
            (post: any) => post.company?._id === companyId,
          );
        } else {
          const explorePosts = await getExploreFeed();
          data = explorePosts.filter(
            (post: any) => post.company?._id === companyId,
          );
        }
      } else {
  ...
  return (
    <section className="mt-14 px-4 sm:px-8 max-w-4xl mx-auto w-full mb-5">

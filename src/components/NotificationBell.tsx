...
const NotificationBell = ({ role }: Props) => {
  ...
  return (
    <div className="fixed top-10 right-6 z-50" ref={ref} >
      {/* Bell Button */}
      <button
        onClick={() => setOpen((o) => !o)}
        className="group bg-amber-400 hover:bg-amber-500 text-black w-14 h-14 rounded-full shadow-xl flex items-center justify-center relative transition"
      >
            ...
            <div className="h-[200px] overflow-y-auto divide-y divide-slate-50">
              ...
              {notifs.map((n) => (
                <div
                  ...
                  className={`relative px-4 py-3 cursor-pointer transition-colors ${!n.isRead ? "bg-amber-50/40" : "hover:bg-slate-50"
                    }`}
                >
                  {!n.isRead && (
                  ...
                  <div className="pl-3">
                    <p
                      className={`text-[13px] leading-snug ${n.isRead
